...
namespace CrusadersConquest.Helpers
{
    internal static partial class Helper
    {
        ...
        public static void DrawLine(this SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 end, Color? color = null, Rectangle sourceRect = default)
        {
            Vector2 dist = end - start;

            float rotation = dist.ToRotation();

            var destRect = new Rectangle((int)start.X, (int)start.Y, (int)dist.Length(), sourceRect.Width);

            Color drawColor = color ?? Color.White;

            spriteBatch.Draw(texture, destRect, null, drawColor, rotation, texture.Size() / 2f, SpriteEffects.None, 0f);
        }
